...
// "cut a release" PR. Every PR that merges to `main` bumps the version and
// files its notes under a dated `## [X.Y.Z] — YYYY-MM-DD` heading; the push to
// `main` then auto-cuts the git tag + GitHub release (.github/workflows/release.yml).
//
// Claude carries the version in THREE places that must agree: `package.json`,
// `.claude-plugin/plugin.json`, and the `the-librarian` entry in
// `.claude-plugin/marketplace.json` (`.plugins[].version`).
//
// ── Checks (working-tree only, no network) ───────────────────────────────────
//   1. CHANGELOG.md has NO `## [Unreleased]` heading or `[Unreleased]:` link.
//   2. All three version files carry the SAME version.
//   3. The TOP-MOST `## [X.Y.Z] — YYYY-MM-DD` heading === that version.
//   4. The date is a real ISO-8601 calendar date.
//   5. A `[X.Y.Z]:` compare-link exists at the bottom for that version.
...
const marketplaceEntry =
  marketplace.plugins.find((p) => p.name === "the-librarian") ?? marketplace.plugins[0];
...
if (!(pkgVersion === pluginVersion && pluginVersion === marketplaceVersion)) {
  failures.push(
    `Version mismatch across the three manifests: package.json=${pkgVersion}, ` +
      `.claude-plugin/plugin.json=${pluginVersion}, marketplace.json=${marketplaceVersion}. ` +
  ...
  if (!strict) {
    failures.push(
      `Top CHANGELOG heading is malformed: "${top.line.trim()}". ` +
        "Expected `## [X.Y.Z] — YYYY-MM-DD` (e.g. `## [0.3.1] — 2026-06-08`).",
    );
  } else {
...
if (failures.length) {
  ...
  console.error(
    "\nThe model: every PR bumps the version (package.json + .claude-plugin/plugin.json " +
      "+ marketplace.json) + files a dated `## [X.Y.Z]` CHANGELOG entry (no `[Unreleased]`); " +
      "merging to main auto-cuts the tag + release.",
  );
  process.exit(1);
}
